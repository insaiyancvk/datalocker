...
def make_csv(d,k,genstr=[]):
    ...
    if len(genstr)!=0:
        for i in genstr:
            dt.append(i)
    df[d]=dt
    df.to_csv("database/{}.csv".format(d),index=False) # exporting the csv file
...
def func_encryptor(text):
    ...
    # encryption of plain text begins, chooses the functions based on the key and returns an encrypted text
    # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    for i in key:
        ...
        #  --------------------------------------------    
            ...
            # checking if the dob/8 digit number already exists
            if len(d)==0:
                d=input("Enter your date of birth (or any random 8 digit number): ")
                dirs = os.listdir("./database") # saving all the existing csv files to 'dirs' list
                # checking if there are any existing csv files with same name
                while (d+".csv") in dirs:
    ...
    final_key = ''.join(str(i) for i in keygen) # making the list of generated key to a string
    ...
    if len(d)!=0:
        if flag:
            # saves the generated strings from function 5 to the database
            print("Saving the generated strings to the database.. ")
            make_csv(d,final_key,gen_str)
            print("Data saved to {}.csv".format(d))
        else:
            # asking the user if they want to save the key to database
        ...
        # For saving only the key to database
